...
import com.jx.pub.services.mapper.LodgerMapper;
...
public class OrderService {
    ...
    public Boolean bookingRoom(Orders order) {
        order.setOrderId(IDUtil.getUUID());
        order.setOrderShowStatus("0");
        order.setOrderStatus("0");
        String nowTime = TimeUtil.getNowTime();
        order.setOrderCreatTime(nowTime);
        order.setOrderUpdateTime(nowTime);
        int i = orderMapper.addOrder(order);
        return i == 1;
    }

    public List<Orders> getNotInOrdersByUserId(String userId) {
        return orderMapper.getNotInOrdersByUserId(userId);
    }
